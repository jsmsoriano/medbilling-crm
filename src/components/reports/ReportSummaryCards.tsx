...
const ReportSummaryCards = ({ filteredData }: ReportSummaryCardsProps) => {
  ...
  const avgDenialRate = filteredData.length > 0 ? 
    (filteredData.reduce((sum, client) => sum + (client.denialRate || 0), 0) / filteredData.length).toFixed(1) : '0';
  const avgSatisfaction = filteredData.length > 0 ? 
    (filteredData.reduce((sum, client) => sum + (client.satisfaction || 0), 0) / filteredData.length).toFixed(1) : '0';
